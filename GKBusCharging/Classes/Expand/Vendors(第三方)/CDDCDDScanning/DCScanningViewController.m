...
- (void)setUpTipView
{
    ...
    _tipLabel.text = @"请将条码/二维码放入框内，即可自动扫描";
...
//    self.flashButton.frame = CGRectMake((DCScreenW - 25) * 0.5 , DCScreenW * 1.1 - 50, 20, 40); //手电筒的尺寸
    [self.flashButton mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerY.mas_equalTo(self.view).offset(ScreenH/3);
...
- (void)setUpJudgmentWithScuessBlock:(dispatch_block_t)openSession
{
    ...
    if(authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied){
        ...
        [DCScanTool setUpAlterViewWith:self WithReadContent:@"您未打开摄像权限。请在iPhone的“设置”-“隐私”-“相机”功能中，找到“《充哈哈》APP”打开相机访问权限" WithLeftMsg:@"知道了" LeftBlock:nil RightMsg:@"前往" RightBliock:^{
    ...
    if ([metadataObjects count] >0){
        ...
        if (metadataObject.stringValue.length != 0) {
            ...
            if (self.scanDelegate && [self.scanDelegate respondsToSelector:@selector(DCScanningSucessBackWithInfor:)]) {
                ...
                WEAKSELF
                if ([self judgeQRCode:metadataObject.stringValue]) {
                    [weakSelf.scanDelegate DCScanningSucessBackWithInfor:metadataObject.stringValue];
...
//                [SVProgressHUD showSuccessWithStatus:[NSString stringWithFormat:@"扫描结果为：%@",metadataObject.stringValue] ];
            }
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{ //异步
        ...
        for (CIQRCodeFeature *feature in features) {
            ...
            str = [NSString stringWithFormat:@"PIC=%@",feature.messageString];
        }
        __weak typeof(self)weakSelf = self;
...
//                        [weakSelf.scanDelegate DCScanningSucessBackWithInfor:str];
                        
//                        [weakSelf.scanDelegate DCScanningSucessBackWithInfor:[NSString stringWithFormat:@"PIC=%@",[arrayM objectAtIndex:1]]];
                        
//                        [weakSelf stopDeviceScanning]; //停止扫描
                        
//                        [weakSelf.navigationController popViewControllerAnimated:YES];
                        
                        if ([self judgeQRCode:str]) {
                            [weakSelf.scanDelegate DCScanningSucessBackWithInfor:str];
                            [weakSelf stopDeviceScanning]; //停止扫描
                        }else{
                            [weakSelf.session startRunning];
...
- (BOOL)judgeQRCode:(NSString *)message{
    //判断是否为正确的【二维码】
    if ([message containsString:saomapandingUrl]) {
        return YES;
    }else{
        [SVProgressHUD showErrorWithStatus:@"请扫描正确的《冲哈哈》产品的租电二维码"];
        return NO;
    }
}
