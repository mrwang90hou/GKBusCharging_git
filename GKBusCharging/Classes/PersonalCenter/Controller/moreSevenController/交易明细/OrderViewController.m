...
- (LBPScrollSegmentView *)scrollSegmentView {
    if (!_scrollSegmentView) {
        _scrollSegmentView = [[LBPScrollSegmentView alloc] initWithFrame:CGRectMake(0,K_HEIGHT_NAVBAR, BoundWidth, BoundHeight) delegate:self titlesGroup:@[@"全部",@"充值记录",@"提现记录"] controllersGroup:@[self.willPayVC,self.unWalkVC,self.willEvaluateVC]];
    }
